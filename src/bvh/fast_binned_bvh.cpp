...
uint32_t FastBinnedBvhBuilder::build(
...
uint32_t FastBinnedBvhBuilder::allocateNodePair()
{
...
void FastBinnedBvhBuilder::subdivide(uint32_t nodeId)
{
...
bool FastBinnedBvhBuilder::partition(uint32_t nodeId)
{
...
AABB FastBinnedBvhBuilder::createBounds(const TriangleSceneData& triangle)
{
    glm::vec3 min = glm::vec3(std::numeric_limits<float>::max()); // Work around bug in glm
    glm::vec3 max = glm::vec3(std::numeric_limits<float>::lowest()); // Work around bug in glm
